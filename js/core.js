(function() {
	 ...
	 */
	$D._leapYear = function(year) {
    return ((year % 4 === 0 && year % 100 !== 0) || year % 400 === 0);		
	}
	
	/* Kinda private */
	$D._daysInMonth = function(year, month) {
    return [31, ($D._leapYear(year) ? 29 : 28), 31, 30, 31, 30, 31, 31, 30, 31, 30, 31][month];		
	}
	...
	$P.add = function() {
		...
		switch (unit) {
			...
			case 'months':
				...
				this.setDate(Math.min(_date, $D._daysInMonth(this.getFullYear(), this.getMonth())));
				break;
			case 'y':
			case 'year':
			case 'years':
				this.add(value * 12, 'months');
				break;
			case 'w':
			case 'week':
			case 'weeks':
				this.add(value * 7, 'days');
				break;
			case 'd':
			case 'day':
			case 'days':
