$(function() {		
	...
	test("date.reset() -> ensures that hours = minutes = seconds = milliseconds = 0", function() {
		...
		same(d, Date.today(), "and is same is Date.today()");
	});
	
	test("that 2009 is no leap year", function() {
		equals(Date._leapYear(2009), false, "2009 is not a leap year");
	});
	
	test("that 2008 is a leap year", function() {
		equals(Date._leapYear(2008), true, "2008 is a leap yar");
	});
	
	test("use add() to add everything :)", function() {
		d = new Date(2000, 0, 1);
		d.add(17); /* days */
		d.add(-1, 'week'); /* subtract a week! */
		d.add('month'); /* add 1 moth */
		d.add(6, 'years'); /* add 6 years */
	...
	test("use add() to add a year during a leap year", function() {
		...
		d.add('year'); // adding a year => 28th Feb 2009 (!)
